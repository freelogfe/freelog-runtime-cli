...
> 仓库：`docs/` · `packages/templates/` · `packages/cli`（见 [12](./12-仓库与发布管理.md)）；旧 `freelog-cli-ts` **不保留**  
...
| `packages/templates/*` | 主题↔插件 **共用** 运行时工程、前端库工程 | 不是「主题专用发行通道」 |
...
| 步骤 | 现状行为 | 问题 |
|------|----------|------|
| 注册表 | `getProjectTemplate()` **硬编码** 11 条 | 与 `packages/templates/` 不同步；版本常写死 `1.0.0`，源包已到 `3.x` |
| 拉取 | `npm install @freelog-cli/template-*` → `~/.freelog-cli/template` | **不读**仓库内 `packages/templates/`；离线/改本地模板需先 pub |
...
解析：compat → `npmName@version` → 缓存；本地 `packages/templates/` 开发时 manifest.`runtimeVersions` 须含所选档。
...
2. 模板根：
   ...
   → <repo>/packages/templates/<id>（开发，且 manifest 含该 runtime）
...
| S1 | 本地 packages/templates/ + npm 精确版本缓存 | 旧 CLI 包内旧 compat 仍指向旧模板 |
