...
| publish | 走与 Console 同序：`PlatformTool.getSHA1Hash` → `Storage.fileIsExist` →（可选）`getResourceBySha1` → upload → `Resource.createVersion`；**无** `--draft` |
| 资源类型 | create/publish **通用**；类型 code 来自平台；脚手架模板可选（主题↔插件共用 runtime 模板） |
| 运行时档位 | 仅主题/插件等运行时类：`runtimeVersion` 必随发版声明；缺省 exit 4；见 [11](./11-脚手架与模板.md) |
| 压缩上传 | 按类型 zip；临时文件 `finally` 删除；磁盘失败 → exit 1 |
| sha1 | 算法/hex **对齐** `FUtil.Tool.getSHA1Hash`；禁止旁路自造指纹 |
...
| online | **严格** resourceOnline：latestVersion + ≥1 启用策略；不代建策略 |
| **软上架分歧** | Console Step4 `update(status:1)` 可弱门禁；CLI **禁止**用 `update` 设 status:1 当上架；软上架资源 CLI `online` 仍按硬门禁拒 |
...
| collection publish | `isMergeCatalogueDraft`；授权缺口 **exit 5**（同构单品）；超时 exit 1 + hint 重试 |
| RSS | 人机混合：`send-code` → 邮箱 → `bind --code`；无 code → exit 4 |
| display | 非法枚举 → exit 4 |
...
| Console 防抖 / 改发版草稿 | `status` 暴露远端有草稿；draft push 走指纹冲突，禁止盲盖 |
| TTY | **不**默认劝「是否 draft push」；跨端由用户显式 push |
...
| 能力 | 原因 | CLI 路径 |
|------|------|----------|
| RSS 收验证码 | 码在邮箱 | 人工 `bind --code` |
| 依赖策略可视化签约（Phase1） | 微应用 | exit 5 + Console hint |
| 策略 Builder | 可视化 | Console 导出 → `--from-file` |
| 向导式一页填完 | 无 wizard | 文档脚本序列 / npm scripts |
| 复刻 300ms 防抖会话 | 非 CLI 模型 | 显式 draft * |
...
- [ ] `--cwd` 下相对路径均相对 cwd  
- [ ] 单品/合集 config 发现未串味  
- [ ] 业务只调 `ServiceAPI` / `PlatformTool`，无旧 `src/api` 分叉路径  
- [ ] publish ↔ draft 语义未混用  
- [ ] 合集 item 未写成「发版草稿」  
- [ ] 错误映射 2/3/4/5  
- [ ] 冻结 bitmask；online 走严格 resourceOnline（无软 status:1）  
- [ ] policyText 单次 encode；policy.json schema 校验  
- [ ] publish exit5 `--json` shape 与命令规格一致  
- [ ] sha1 占用无交互确认、仅 warn  
