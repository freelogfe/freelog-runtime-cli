...
| **L6** | CLI 命令 + service 函数 | `freelog-cli publish` → `publishService.publishVersion` |
...
| `TOP-RB-NAMES` | generateResourceNames | Resource.generateResourceNames | fromDirService | ✓ | ✓ | — | 26 |
...
**publish 前：** `authTree`（FGraph 组件 + CLI publishService）— A✓ B✓ #67。
...
| CLI 合集 | `packages/cli/src/services/collectionService.ts` |
