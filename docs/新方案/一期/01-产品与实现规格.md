...
> **文档角色：** 一期（资源发行域）**产品与实现规格摘要**——定义能力、命令、门禁、service、伪代码和 exit code。用户体验细节、完整场景拓扑和手测前 UX 验收以 [02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md) 为真源。
...
```text
改用户体验 / 步骤顺序     → 02-CLI体验拓扑设计.md；本文同步摘要
...
一期资源发行域的用户目标不是“复刻 Console 页面”，而是让用户或 AI 在本地电脑上安全、快速、可恢复地完成资源发布、更新、批量整理、合集维护和线上上下架。完整用户目标与体验拓扑见 [02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md)。
...
| 策略与上下架 | P | TTY 目标：policy template/list/render/apply；当前 fallback：policy apply/set/list/init；online/offline |
| 合集 | C | collection 全流程 |
...
| 节点壳写、展品、收入 | 不注册命令 | Console；二期 exhibit 见 [二期/](./archive/二期/) |
...
### 1.4 主资源类型入口（一期开工前必须覆盖）

三种发行模式之下，`init` 和手测必须继续覆盖真实资源类型入口。完整剧本见 [场景/](./场景/README.md) S01–S14；总拓扑只在 [02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md) 保留跨场景规则。

| 入口 | 典型用户 | 模板 / 类型处理 | 必须覆盖的体验 |
|---|---|---|---|
| `init theme` | 主题开发者 | runtime 模板；平台「主题」叶子类型 | 模板选择、runtimeVersion、build dist、directory zip、policy/online |
| `init widget` | 插件开发者 | runtime 模板；平台「插件」叶子类型 | 独立插件入口，不能靠搜索；文案不能写成主题 |
| `init package` | 前端库开发者 | package 模板；模板映射 JS工具包/组件库叶子 | namespace、package 模板、dist 发行；package 是 CLI-native 一等入口 |
| `init other` | 普通内容作者 | 逐级选择任意可本地上传叶子类型 | file / directory-zip、格式/MIME/大小/SHA、属性解析 |
| `resource import-dir` | 批量运营 | 默认类型或逐项类型 | 预览、分批、report、恢复 |
| `init collection` | 策展人 | 合集 subjectType；合集类型树 | 目录草稿、合集版本、display、RSS/collect-rules |
...
| ID | 流程摘要 | 能力 | 公开文档 |
|---|---|---|---|
| **J-01** | init → create → validate → publish → dep? → 策略模板/启用策略 → online | R/V/D/P | [使用/快速上手](./使用/快速上手.md) |
...
| **J-05** | J-01 + 二期 exhibit（跨期） | — | [二期/11 J-05](./archive/二期/11-完整用户旅程.md) |

表中的箭头是工程/AI/CI 可复制的 checkpoint 序列，不是普通 TTY 用户必须手动背诵的命令清单；普通 TTY 主路径由 [02 §2.2](./02-CLI体验拓扑设计.md) 的连续向导串联这些 checkpoint，逐场景细节见 [场景/](./场景/README.md)。

端到端步骤详表见 [02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md)；跨期视角见 [二期/11 §2–§5](./archive/二期/11-完整用户旅程.md)。**改旅程顺序或新增分支时先改 02，再同步本文 §4 与二期/11。**
...
一期重构后的普通用户主路径以 [02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md) §0.2 为准：

```text
输入一个入口命令 → 选择任务/资源类型 → 按 Console 约束补字段 → preflight → 确认写入 → 继续下一步或安全退出
```

这意味着 `init`、`publish`、`policy`、`online` 等显式命令是 checkpoint 和专家快捷入口；普通 TTY 用户不应被迫记住 `create → version set → publish → 策略模板 → online` 这条完整链。非 TTY / AI / CI 仍走显式命令、manifest、`--json`、`--yes` 和 `--env`。

### 3.1 双表面：人类 vs 机器

| 表面 | 触发 | 输出 | 写操作 |
|---|---|---|---|
| **TTY** | 入口命令、缺必填、缺 `--yes` | 连续向导 + preflight + confirm | 可交互补全；每个 checkpoint 可退出/恢复 |
...
| 场景 | Console | CLI 体验设计 |
|---|---|---|
| 首发 Step4 直接上架 | 可能无 latest/policy 检查 | **拆步** 策略模板/启用策略 → online；online 走 **sidebar 严格门禁** |
...
交互拓扑与手测前 UX 验收：[02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md)；字段级规格：[开发/CLI交互与字段约束.md](./开发/CLI交互与字段约束.md)。
...
| dep auth | 依赖可发行 | 合同 | **5** |
| policy template/apply | 访客可授权 | policyId | 4 |
...
collection create → item add|import-dir → item reorder（即时 draft API）
  → collection publish（目录 merge 语义）→ 策略模板/启用策略 → online
```
...
| 当前 `policy apply|set|list|init`；目标 `policy template list/render` + `policy apply --template` | ✓ | policyService；目标需新增 policyTemplateService | P-01/P-02 |
...
// resume: 只处理 remote_succeeded_local_pending；retry 只处理 failed。
// remote_outcome_unknown 必须停止并要求按 owner / version / SHA 人工对账。
```
...
|---|---|
| `schemaVersion` | 1 |
| `command` | **应为完整命令路径**（已知债务：部分命令仅顶层名，见 [审计 F-001](./archive/审计/CLI一期深度审计报告.md)） |
...
| 变更类型 | 先改 | 再同步 |
|---|---|---|
| 用户步骤顺序 / 新旅程分支 | 02-CLI体验拓扑设计 | 本文 §2–§4、二期/11、使用/ 相关页 |
...
**2026-08-28 更新：** 一期体验细节已拆到 [02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md)；本文保留能力、命令与门禁摘要。
...
| 审计 | [审计/CLI一期深度审计报告.md](./archive/审计/CLI一期深度审计报告.md) |
| 二期 | [二期/README.md](./archive/二期/README.md) |
