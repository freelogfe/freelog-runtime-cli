...
表中的箭头是工程/AI/CI 可复制的 checkpoint 序列，不是普通 TTY 用户必须手动背诵的命令清单；普通 TTY 主路径由 [02 §0.2](./02-CLI体验拓扑设计.md#02-产品形态裁决tty-连续向导优先) 的连续向导串联这些 checkpoint，逐场景细节见 [场景/](./场景/README.md)。

端到端步骤详表见 [02-CLI体验拓扑设计](./02-CLI体验拓扑设计.md)；跨期视角见 [二期/11 §2–§5](../二期/11-完整用户旅程.md)。**改旅程顺序或新增分支时先改 02，再同步本文 §4 与二期/11。**
