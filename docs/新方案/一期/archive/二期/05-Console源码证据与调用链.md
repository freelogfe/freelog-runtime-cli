...
> 文档角色：Console 侧 API 与调用链证据。**§A 资源域**（一期）+ **§B 节点域**（二期）+ **§C 测试对照**（QA 并排）自洽摘要；细粒度 resource 拓扑在仓库 `docs/新方案/一期/对齐/`（本目录不修改）。
...
**测试：** 一期资源 CORE 的 Console 并排见仓库 `docs/新方案/一期/对齐/`（本目录不修改）。二期 NM 并排见 [05 §C](./05-Console源码证据与调用链.md#c-测试人员-console-对照nm-core)。
