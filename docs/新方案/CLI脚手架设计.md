...
| [用户场景与问题清单](./用户场景与问题清单.md) | 全路径穷举、边界问题、命令实例 | **3**（排查） |
| [CLI使用说明与Console差异](./CLI使用说明与Console差异.md) | 命令参数与 Console 差异 | 按需 |
...
联调账号见 [CLI交接文档 §4](./CLI交接文档.md#4-环境和账号)。复验命令：
...
完整 parity 清单见 **§1.9**；Console 页面 → API → CLI **操作级溯源**见 [CLI数据操作与Console对照](./CLI数据操作与Console对照.md)（**主真源**）；本节 **§1.10** 保留维护期速查表。不在 parity 范围内：列表浏览、收藏、加节点、交易收入、详情页预览、策略 Builder UI、视频转码。
...
Console 对资源的**数据操作**分两大阶段，CLI 均须 parity（本地文件 → manifest → API）：
...
**完整 81 项业务清单 + 逐项状态见 [CLI数据操作与Console对照 §0–§2](./CLI数据操作与Console对照.md)** — **唯一真源**，本文不再重复状态表。

**结论（2026-08-06）：** 38✅ / 12⚠️ / 5❌ → **未对齐**；P0 = PropertyParser 全链路。代码任务：对照表 §3。
...
#### 1.9.6 本地发版 — 文件属性解析（**核心未对齐**）

Console 对 **每一种本地文件**（图片、视频、zip、音频…— 由 `resourceTypeCode` 决定解析项，不是视频专属）在 **createVersion 之前** 必走同一套属性链路：

```text
本地文件上传得 sha1
  → handleData_By_Sha1_And_ResourceTypeCode_And_InheritData2
  → PropertyParser SSE /v2/storages/files/listSSE/info
  → metaInfoArray（平台按类型配置：insertMode 1=raw 只读, 2=additional 可编辑）
  → getAttrsInfoByKey（格式校验）
  → systemProperties（UI 称「系统属性 / 补充属性」）
  → createVersion / createBatch 提交：
       inputAttrs ← systemProperties 中 type==='additional'
       customPropertyDescriptors ← customProperties + customConfigurations
```

**Console 调用点（凡本地文件发版均有）：**

| 入口 | 源码 |
|---|---|
| 单品首版 creator Step2 | `resourceCreatorPage/step2Effects.ts` · `onSucceed_step2_localUpload` |
| 发新版 versionCreator | `resourceVersionCreatorPage.ts`（换文件 / 继承上一版属性） |
| 批量 creatorBatch | `creatorBatch/Handle/index.tsx` · `handleLocalUploadSuccess`（**每个文件**） |
| 合集 Step2 本地上传子资源 | `collectionCreatorPage/step1Effects.ts` |
| sidebar 改已发版属性 | `resourceVersionEditorPage.ts` · `syncAllProperties` → `updateResourceVersionInfo` |

源码：`console/src/utils/PropertyParser.ts`、`console/src/utils/service.ts`。

**CLI 现状 vs Console 数据操作：**

| Console 写入字段 | CLI |
|---|---|
| 上传 + fileSha1 + filename | ✅ |
| PropertyParser → systemProperties → **inputAttrs** | ❌ **整条链路缺失** |
| 类型可选配置 → **customPropertyDescriptors** | ❌ 自动链路缺失；manifest **手填**可提交 |
| 用户「补充属性」表单编辑后再提交 | ❌ 无 UI；手填 manifest 等价 |
| `updateResourceVersionInfo` · inputAttrs + customPropertyDescriptors | ❌ `version edit` 仅 description |

**结论：** 当前 CLI 只提交了 createVersion 的 **文件三件套 + manifest 手填字段**，**没有**复刻 Console 在提交前对 **所有本地文件** 做的属性解析与组装。这是相对 Console **本地文件数据操作** 的最大缺口；dev 场景（S6 图、S10 视频、S7 zip）能过，只说明 **当前 dev 类型未强制校验完整属性**，不能证明已对齐。

**对齐目标（P2）：** 在 `publish` / `import-dir` / `collection item import-dir` 上传完成后，调用与 Console 等价的 `handleData_By_Sha1…`，把结果写入 manifest/state，再随 createVersion/createBatch 提交；维护期补全 `version edit` / `syncAllProperties` 等价能力。
...
| 修改已有策略正文 | 新增策略 + 启停 |
| 视频转码 | 上传原文件 |
| creator Step4 软 `status:1` | CLI 用严格 `online`，见 §0.5.5 |
| Console 300ms 自动 `saveVersionsDraft` | CLI 须显式 `draft push` |

### 1.10 Console 页面 → API → CLI 溯源（维护期速查）

**完整数据操作明细（请求体字段、草稿分类、上传链路、策略语法、dev 实测）见 [CLI数据操作与Console对照](./CLI数据操作与Console对照.md)。** 下表仅作维护期快速索引；接口签名见 `tools-lib/src/service-API/resources.ts`。
...
| Console 模式 | 主要模块 |
|---|---|
| 发行单个资源 + init | `initWizard`, `initCatalog`, `resourceTypePicker`, `scaffold`, `publishService` |
| 批量发行资源 | `fromDirService`, `commands/resource` |
| 发行合集 | `collectionService`, `fromDirService` |
| 发布 | `publishService`, `processFile`, `policyService`, `onlineService` |
| 同步 | `syncService`, `config/project` |
