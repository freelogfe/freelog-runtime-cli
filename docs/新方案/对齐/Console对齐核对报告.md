...
> 文档角色：2026-08-10 历史验收快照。它记录当时采用的范围、脚本和结论，不定义当前产品范围，也不代表当前 commit 已通过。当前产品设计见仓库根目录 [DESIGN.md](../../../DESIGN.md)，当前能力证据见 [CLI数据操作与Console对照](./CLI数据操作与Console对照.md)。

最后更新：2026-08-10

**读者：** 产品、测试、开发  
**当时使用的核对依据：** 旧版 81 项清单与 [Console完整业务梳理](./Console完整业务梳理.md)。当前契约以 [Console–CLI 业务能力契约](./CLI数据操作与Console对照.md) 为准；本文数字不得用于当前完成声明。
...
**当时结论：** 在该报告采用的范围和证据口径下通过。该结论不得脱离具体 commit、环境、跳过项和契约版本用于当前发布签字。
