...
命中最近的工作区文件后必须成功读取；文件损坏、缺少 token 或无法解密时会明确失败，
不会静默换用祖先或全局账号。用户主目录的 `~/.freelog-auth` 始终按全局凭据处理。
...
2. **`login`（默认）** 会先在当前目录 `.gitignore` 末端建立有效的 `/.freelog-auth` 忽略规则，再原子写入凭据；**`login --global` / `-g`** 写入用户主目录。
...
10. `.freelog-auth` 必须 gitignore，不得提交仓库；即使旧规则曾用 `!.freelog-auth` 反选，login 也会在末端重新强制忽略。
