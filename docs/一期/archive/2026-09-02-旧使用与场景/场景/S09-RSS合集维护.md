...
> **文档角色：** RSS / feed 与合集绑定、同步、锁定字段的历史场景材料。当前主线方案见 [../产品方案/README.md](../产品方案/README.md)；交互拓扑补充见 [../02-CLI体验拓扑设计.md](../02-CLI体验拓扑设计.md)。
...
| 字段锁定 | title/cover/intro/display/items/status/version 草稿等 feed 托管项必须拒绝；tags 当前允许维护 |
...
| R2 inspect | feed URL、标题、matchedItemCount、风险 | URL | 只读平台/feed | feed 不可达给原因 |
| R3 验证码 | 需要 ownerEmail/外部确认 | send-code/输入 6 位 code | 触发平台验证码 | 验证码错误停在 code 字段 |
| R4 bind | 将绑定 feed 到合集 | confirm | 写平台/state | 已绑定则区分自己可继续/他人阻断 |
| R5 status/sync | 同步状态、失败项、锁字段 | sync/退出 | 可能写平台 | 远端错误显示 item；防重复 sync |
...
| 用户想改 tags | 允许，继续使用普通标签规则 |
| 用户想改 display/items/update status | code 4，说明 RSS feed 托管并给 status/sync |
| matchedItemCount 超过 1000 | 要求日期范围，按日开始/日结束重新 preview |
...
- 锁定字段是硬门禁，不是 UI 提示；tags 是当前 Console 明确允许的例外；
