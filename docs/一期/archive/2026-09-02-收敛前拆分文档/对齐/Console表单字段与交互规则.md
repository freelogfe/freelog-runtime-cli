...
> 当前入口：跨流程阅读先看 [Console业务流程字段接口总账](./Console业务流程字段接口总账.md)；本文只作为字段级明细补充。

证据快照：2026-09-01，Console 分支 `dev@0.19.0`，HEAD `acc1f43f8`；RSS 提交重点复核 `a0c2df41e`。相对上一快照已复核 RSS 绑定/换源、合集 Step4、合集 info、普通资源 info 与资源卡片；RSS 相关资源的 tags 允许编辑规则有语义变化，已按当前源码修订。TTY 逐步交互规格见 [CLI交互与字段约束](../开发/CLI交互与字段约束.md)。
...
| `FORM-LIST-RSS-LOCK` | sidebar info · RSS 关联资源 | `CONDITIONAL` 当前 Console 锁定标题、封面、简介等 feed 托管内容；**标签 tags 允许继续编辑** | Resource update | CLI 写入前必须识别相同平台限制；不能把 tags 错误归入 RSS 锁字段 | 待专项 ENV |
...
| `FORM-POL-TEMPLATE` | fPolicyBuilder3 · 模板列表 | `HARD UX` 先 `Policy.policyTemplates` 拉模板，选择后可编辑；模板按资源类型/节点上下文过滤，含译文、代码和 `reportUiTemplate` 参数 | `Policy.policyTemplates`、`policyReCompile`、`policyTranslation` | TTY 新增策略必须先展示模板并填参数；`--from-file` 只作 advanced/AI/CI fallback | 基础 Builder 已实现，ENV 待完整签字 |
...
| `FORM-COL-RSS` | RSS 绑定和同步 | 地址预检、owner email、重复占用、1000 条阈值与日期范围、验证码、换源 GUID 比对、同步进度/失败项 | Rss APIs | `collection rss inspect/send-code/bind/status/sync`；危险换源须 `--force --yes` | CONTRACT 对齐；受控邮箱 ENV 待执行 |
| `FORM-COL-RSS-LOCK` | RSS 合集维护 | 标题、封面、简介、展示设置、目录项、更新状态均禁用；**标签 tags 允许继续编辑**；**versionInfo 草稿 look/save 亦跳过** | Resource/Rss APIs | service guard + 无 version draft；tags 使用普通标签规则 | CONTRACT 已复核；CLI 代码与 RSS ENV 均须专项验证 |
