...
| resourceTitle/authIdentity operator | INCLUDES / NOT_INCLUDES / STARTS_WITH / ENDS_WITH |
| resourceTypeCode operator | EQUAL |
...
| tags | 标签编辑器，最多 20 个、单项最多 20 字；RSS 相关资源当前仍允许编辑 | `form_input_tag_error_length` 等 |
...
| RSS sync | Rss.syncBinding；进行中或 sync 请求后等待 progress 刷新时防重复 | `collection rss *` |
| Sider 上下架 | **同款** `resourceOnline` / status:4（`collectionSidebar/Sider/index.tsx` L277+） | `collection online/offline` 或 resource `online` |

**合集 RSS 锁定（当前 Console）：** 普通 RSS 相关资源与 RSS 合集 info 页均禁用标题、封面、简介；RSS 合集还禁用展示设置、目录项与更新状态等 feed 托管能力。标签 tags 当前仍允许编辑，不能继续按旧结论视为锁字段。

**合集 collect-rules 维护（sidebar info · 非 creator Step4）：** 编辑态 Save 调 `setCollectRules`，字段与 creator Step4 同构；`STARTS_WITH` 操作符值自动前缀 `username/`（维护页路径）；Save 禁用条件：任一 condition `value===''` 或 `valueError` 非空。
