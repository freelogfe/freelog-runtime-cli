...
| P-01 | 新增策略 | TTY `policy template` Builder → `policy template apply` / `policy apply --template`；AI/CI `policy template render`；advanced `policy apply --from-file`；会话 **`policy template apply --session`** | CORE / EQUIVALENT | SPEC+CODE；ENV 待完整场景签字 | Console 先模板列表、参数编辑、编译和译文预览；最终正文 URI 编码；重复检测；advanced 追加策略额外校验 `FOR PUBLIC`+`Initial:`（`FORM-POL-APPEND`） |
...
| C-10 | RSS 绑定与同步 | `inspect/send-code/bind/status/sync` | ADVANCED / PARITY | SPEC+CODE+CONTRACT；ENV mandatory | 预检、1000 条阈值、换源 GUID、同步状态，以及标题/封面/简介/目录展示等 RSS 托管字段锁定；tags 当前允许维护；`verify:rss` 受控邮箱状态链方可签字 |
