...
| `FORM-LIST-RSS-LOCK` / `FORM-COL-RSS-LOCK` | RSS 关联合集 | 标题/封面/简介、合集展示/条目/更新状态等 feed 托管字段 disabled；tags 当前允许编辑 | `isRssRelatedResource` → 只阻断被 Console 锁定的字段；tags 使用普通标签校验 | ✅ P2/P5 update 向导预检需按新 Console 规则复核 |
...
| 2 | 类型树逐级 pick / 搜索 | `FORM-RES-TYPE` | Step1 类型必选 | 须 login；叶子类型说明；默认从一级级浏览开始，搜索只是快捷入口 | 平台树 + 拒绝非叶子 | ✅ picker | 保持；主线见 [命令与交互设计](../产品方案/02-命令与交互设计.md)，字段见 [字段约束总账](../产品方案/07-字段约束与提示总账.md) |
...
[一期产品方案 · 命令与交互设计](../产品方案/02-命令与交互设计.md)；字段约束见 [字段约束总账](../产品方案/07-字段约束与提示总账.md)
