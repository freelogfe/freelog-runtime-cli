...
`[dir]` 省略时，TTY 询问“项目目录”；非 TTY 必须显式给出 `[dir]`，避免脚本意外向当前目录写入。模板模式目标可以不存在或为空，唯一允许的非空例外是 `.freelog/auth`。通用接入模式的目标可以是已有项目，但必须尚无资源 `N.json` / 工作稿 / index；若已有 `.freelog/`，其中只能有认证选择器 `.freelog/auth`。`init` 只在该目录新增 `.freelog/1.json`，不覆盖或合并任何业务文件；符号链接歧义、已有身份或不可写时失败。`--yes` 只跳过确认，绝不是覆盖/合并开关；本期没有 `--force`。
