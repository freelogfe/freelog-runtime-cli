...
本期单资源范围只做**独立单资源**（`subjectType=1`）。一个工程可有多份独立资源状态；合集是另一个 `subjectType=4` 主体，其正式设计见 [合集](../合集/README.md)，当前尚未实现。archive 中的旧合集稿不再是设计依据。
...
| 批量发行 | [01-数据协议与执行](./批量发行/01-数据协议与执行.md)（本轮讨论稿，尚未实现；独立于合集） |
