...
本期只做**独立单资源**（`subjectType=1`）。一个工程可有多份独立资源状态；合集暂缓，见 [archive](../../../../archive/2026-09-04-脚手架设计-合集备份/README.md)。

### 所有 PHASE 的资源路由前置条件

本目录的每一份资源命令设计均继承 [08 的命令路由矩阵](../../ARCHITECTURE/08-多资源本地状态、选择与产物路径.md#31-命令路由矩阵)，不得在子文档另造默认选择规则。常规身份命令必须覆盖零/一/多状态、TTY 退出、非 TTY/`--yes` 与显式选择器；`create` / `bind` 必须使用新增/接续状态的专属路由；`resource sync` 是唯一省略选择器即批量操作的例外。子文档只补充命令本身的业务门禁，不能覆盖这条前置条件。
