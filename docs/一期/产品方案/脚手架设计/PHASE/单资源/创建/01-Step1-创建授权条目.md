...
须已 `login`。`--yes` 不进下面任何一步提问，必须给 `--title` / `--name`；仅当对上的工程没有已验证 `typeCode` 时才必须再给 `--type`。每份有效本地状态已有必填 `filePath`；因此无可续接身份时，`create --yes` 还必须带 `--artifact`，并在 POST 前确认其存在且匹配类型。`init theme` / `init widget` 已写固定 `RT001` / `RT002`，创建壳时不得要求用户重复选择或传入类型；本期没有模板元数据缓存。

已有的主题/插件项目有两条合法入口：要先建立不连平台的本地状态，运行 `init --type RT001|RT002 --artifact <文件或构建目录>`；要一步到位则运行 `create --type RT001|RT002 --artifact <文件或构建目录>`。两者都不复制模板、不渲染；前者只写未绑定身份，后者才 POST 创建资源壳。普通已有文件项目同理可先 `init --type <leaf> --artifact <文件>`。
...
已 login
  ...
  → 4. 对应产物 `--artifact`（新增身份时必填；先验存在与形态，只记路径，本步不上传）
...
新增或改写锚点的路径必须落在当前工程里、当前已经存在，并在建壳 POST 前通过类型形态校验；本步不上传该产物。
...
路径必须落在当前工程里且当前存在：普通资源只能是文件；主题/插件可为文件或构建目录。本步不调 Storage、不上传。
