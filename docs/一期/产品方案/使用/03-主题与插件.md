...
| 模板缓存 | 模板初始化时有 `N.template.json`；既有工程没有也合法 | 模板初始化时有 `N.template.json`；既有工程没有也合法 | 没有 |
...
非空的既有项目不能再运行 `init`，也不会有模板缓存。若它还没有线上资源壳，在项目根直接建壳，并显式记录构建目录：
...
主题/插件既有项目路径都会把 `dist` / `build` 记录为目录，并在发版时临时压缩；不会补造 `N.template.json`，也不会覆盖你的源码。
