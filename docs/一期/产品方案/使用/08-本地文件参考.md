...
| `filePath` | 相对工程根目录、以 `/` 分隔。普通资源是文件路径；主题/插件可以是构建目录（例如 `dist`）或单个文件（包括 zip）；绝对路径与 `..` 越界路径不合法。 |
...
`N.json` 不要求用户手工记编号。所有单资源命令可用 `--resource id:<资源ID>`、`name:<标识>`、唯一的 `title:<标题>` 或 `artifact:<filePath>` 选中它；`file:N.json` 是仍完整支持的精确状态文件形式，适合 AI、脚本和恢复。
