...
当前后端按资源类型筛选模板存在故障，`policy template list` 暂时列出平台返回的**全部**模板（默认每页 20 条），包含带交易事件的模板，不在 CLI 按免费/付费、事件或类型猜测过滤；后端修复后再恢复按类型请求。交互执行 `policy template apply` 时可在每页选择、翻页或取消；脚本执行必须提供 `<templateId> --yes`。应用模板会先由平台重新编译，最终能否写入由平台判定；目前带事件或需要填写时间、金额等参数的模板可能受后端编译契约阻塞，CLI 不猜测转换。应用模板或 `policy apply` 都会新增一条**启用**的授权策略，但不会触发支付，也不会自动上架。
