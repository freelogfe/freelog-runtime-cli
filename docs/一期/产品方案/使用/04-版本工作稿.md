...
多资源工程中，本页所有命令都加同一个 `--resource <selector>`：人工优先 `id:`、完整 `name:` 或 `artifact:<当前文件>`；AI、脚本和恢复可用精确的 `file:N.json`。标题只展示，不能选择资源。例如 `version draft pull --resource artifact:video.mp4` 是选择视频资源，不是上传文件。
...
freelog-cli version attr review
...
可选配置是否可用由资源类型详情的 `supportOptionalConfig` 决定；当前类型不支持时，`version option add` 和 `set` 会停止，`rm` 仍可用于清掉历史工作稿的遗留项。若遗留项没有清掉，首版和更新版提交前还会再次拒绝，绝不会悄悄送到平台。属性和配置的键创建后不可改；需要改键时删掉后重新添加。命令会在写入前显示预览，`--yes` 只跳过确认，不跳过字段校验。

如果换文件或重新分析后某个系统附加属性不再存在，CLI 会保留旧值并阻止提交，不会悄悄删除。先查看：

```powershell
freelog-cli version attr review --env dev
```

若下一次分析又出现同一 `key`，旧值会自动恢复。只有明确不再需要时，才逐项丢弃：

```powershell
freelog-cli version attr review discard <key> --yes --env dev
```
...
发版总是以当前存在的本地文件或构建目录为准；不能用旧工作稿 SHA1 代替丢失文件。换文件时在 `create-version` / `update-version` 上给新的 `--artifact`。一个工程有多份资源时，先用 `--resource <selector>` 选中身份，再用 `--artifact <新路径>`；普通资源给文件，主题/插件给文件或构建目录（仅目录会临时压缩）。主题/插件目录中的软链接不会被打进包，需先替换为实际构建文件。新脚本不要使用 `--file`。
