...
一个工程可管理多份独立资源身份；每一份身份各自有一个默认 `filePath`。`--artifact` 只表示**本次选中资源**的产物路径，绝不选择资源。多份状态时先传 `--resource file:N.json`、`id:<资源ID>`、`name:<短标识或username/name>` 或 `title:<标题>`；不传时交互终端列出完整信息供选择，非交互会列出可复制的 `file:N.json` 示例并停止。`create` / `bind` 用 `--artifact` 记录将来默认使用的路径，路径可以暂时不存在；`create-version` / `update-version` 用它指定本次文件或目录，发版产物必须真实存在：
