...
- 发版前改文件可附 `--file <文件或构建目录>`；成功确认后该路径会回写身份记录。
...
- 多份身份共用一个工程时，`--file` 既用于定位本地身份，也用于记录实际文件或构建目录。
...
`--file` 从不表示“复用旧 SHA1”。文件必须在本地存在：

| 场景 | 传入的路径 |
|---|---|
| 普通资源 | 一个实际文件。目录会被拒绝。 |
| 主题 / 插件 | 一个构建产物目录，例如 `dist` 或 `build`。zip 文件会被拒绝。 |
| 多资源工程 | 已记录在某份身份中的路径，用于选中那份资源。 |

如果未传 `--file`，CLI 只会在已记录路径确实存在时采用它；`--yes` 遇到缺失路径会失败，要求显式给出路径。
