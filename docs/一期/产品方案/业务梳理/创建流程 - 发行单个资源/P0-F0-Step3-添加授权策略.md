...
| Console | CLI | 决策 |
|---------|-----|------|
| 免费/商业/自定义模板 + builder | `policy template list` → `policy template apply <id>`；高级 `policy apply --from-file`。列表保留全部适用模板（包括 `TransactionEvent`），不按免费/付费过滤 | ⚠️ 不做支付、策略编辑器或执行预览（旧文提的 `policy init` 已不在新命令面） |
| 付费 / 支付 | Console | ❌ |
