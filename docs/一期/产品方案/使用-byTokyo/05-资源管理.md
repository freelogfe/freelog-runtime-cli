...
如果你已经成功发布了 `1.0.0`，只需要完成下面三步：

```powershell
freelog-cli policy apply --from-file <策略文件绝对路径> --yes --env dev
举例子：freelog-cli policy apply --from-file "C:\Users\Administrator\freelog-video-demo3\free-policy.json" --yes --env dev
freelog-cli validate --for online --env dev
freelog-cli online --yes --env dev
```

策略应用成功不等于已经上架：第二步只检查条件，第三步才真正修改线上状态。若 `validate` 报错，先按报错补齐版本或启用策略，不要跳过检查。
...
当命令同时使用 `--cwd <工程目录>` 时，`--from-file .\xxx.json` 仍按**启动终端的当前目录**解析，而不是按 `--cwd` 解析。最稳妥的写法是传策略文件绝对路径；另一种做法是先 `cd` 到工程目录，再传相对路径。
...
当前 CLI 的 `online` 成功时可能不输出“已上架”提示；命令以退出码 `0` 返回即表示平台更新已接受。建议紧接着在 Console 查看资源状态。若 CLI 后续增加状态回显，应输出资源 ID 与最终 `status=1`，避免用户误以为命令未执行。
