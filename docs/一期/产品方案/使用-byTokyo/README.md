...
不需要先读完全部文档。请按你的目标选一条：

1. 想发布一个图片、视频或其他普通文件：先看 [快速开始](./01-快速上手.md)。
2. 想发布一个主题或插件：先看 [主题与插件](./03-主题与插件.md)。
3. 已经发布，只想设置策略或上架：看 [资源管理](./05-资源管理.md)。

完成一条流程后，再按需要查询下面的专题文档即可。

## 先安装 CLI

本文档集适用于 `@freelog-cli/cli2@0.5.3`：它使用 `.freelog/1.json`、`create-version` 与 `update-version`。普通资源 CLI 要求 Node.js 20+；Vite 主题/插件模板要求 Node.js `20.19+` 或 `22.12+`。确认版本后，用 npm 全局安装指定版本：
...
`freelog-cli --cli-version` 必须输出 `0.5.3`。如果提示缺少 `freelog.manifest.json`，或帮助中显示 `publish` / `version set`，说明当前 Node 环境命中了另一套 CLI，不能使用本目录文档。切换 Node.js 后应重新执行安装和版本检查。
