...
> 任何命令都可以直接 `node dist/index.js <command>` 运行；若希望更简短，可在项目根执行 `pnpm freelog-cli:<cmd>` 等自定义 script（自行在 package.json 添加即可），无需 `pnpm link`。
...
4. **开发命令**：修改 `src/commands/**` 后由 `pnpm dev` 自动编译，随后运行 `node dist/index.js <cmd>` 验证。
...
- 快速回归：为常用命令封装 npm scripts（例如 `"cli:login": "node dist/index.js login -g"`）即可复用，不必使用全局 link。
