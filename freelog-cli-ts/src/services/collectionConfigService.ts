/**
 * 合集配置文件服务
 * 负责读取、验证和更新 freelog.collection.config
 */
 ...
 * 获取合集配置文件路径
 */
export function getCollectionConfigPath(customPath?: string): string {
  if (customPath) {
  ...
  throw new ConfigError('找不到合集配置文件 (freelog.collection.config.*)');
}

/**
 * 加载合集配置文件
 */
export async function loadCollectionConfig(customPath?: string): Promise<CollectionConfig> {
  const configPath = getCollectionConfigPath(customPath);
  ...
  try {
    ...
    if (error instanceof Error) {
      throw new Error(`加载合集配置文件失败: ${error.message}`);
    }
    throw error;
  }
}
 ...
 * 验证合集配置文件
 */
export function validateCollectionConfig(config: any): asserts config is CollectionConfig {
  const errors: string[] = [];
  ...
  if (errors.length > 0) {
    throw new ValidationError(`合集配置文件验证失败:\n${errors.map((e) => `  - ${e}`).join('\n')}`);
  }
}
...
async function updateConfigFromTemplate(
  ...
  const templatePath = getTemplatePath('freelog.collection.config', format);
 ...
 * 基于模板更新，保留注释和格式
 */
export async function saveCollectionConfig(config: CollectionConfig, customPath?: string): Promise<void> {
  ...
  try {
    ...
    if (customPath) {
      ...
      try {
        configPath = getCollectionConfigPath();
      } catch {
        // 配置文件不存在，使用默认路径（优先 .js）
        configPath = path.join(process.cwd(), 'freelog.collection.config.js');
      }
    }
    ...
    if (error instanceof Error) {
      throw new Error(`保存合集配置文件失败: ${error.message}`);
    }
    throw error;
  }
}
...
/**
 * 将 CollectionConfig 转换为 CreateResourceBody
 * API 使用 name（不是 resourceName）和 resourceTypeCode（不是 resourceType 数组）
 */
export function collectionConfigToCreateBody(config: CollectionConfig): CreateResourceBody {
  if (!config.resourceName) {
    throw new ValidationError('创建合集时 resourceName 是必填的');
  }
  ...
  if (!resourceTypeCode) {
    throw new ValidationError('创建合集时 resourceTypeCode 是必填的（可通过 resourceTypeCode 或 resourceType 数组提供）');
  }
...
export function calculatePolicyChanges(
  localPolicies: CollectionConfig['policies'],
...
/**
 * 将 CollectionConfig 转换为 UpdateResourceBody
 * 注意：此函数不计算 policies 差异，需要先调用 calculatePolicyChanges
 */
export function collectionConfigToUpdateBody(
