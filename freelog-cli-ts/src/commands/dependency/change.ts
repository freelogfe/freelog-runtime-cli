...
import { requireAuth, getCurrentAuth } from '../../core/auth';
import { readConfig, updateConfig } from '../../core/config';
import { CommandOptions } from '../../types';
import { selectVersion } from '../../utils/version-selector';

// 解析资源标识符
function parseResource(resource: string): { value: string; version: string; type: string } {
  // URL 格式
...
// 修改依赖 - 合约应用修改
async function modifyByContract(existingDep: any, targetVersion: string, resourceInfo: any): Promise<any> {
...
// 修改依赖 - 重新签约
async function modifyByResign(existingDep: any, targetVersion: string, resourceInfo: any, auth: any): Promise<any | null> {
  ...
  // 选择策略
  console.log('\n可用策略:');
  const policyChoices = policies.map((p: any) => ({
  ...
  const selectedPolicy = policies.find((p: any) => p.policyId === selectedPolicyId);
  ...
  try {
    ...
    let checkSpinner: Ora | null = ora('正在验证授权...').start();
  ...
  try {
    ...
    try {
      ...
      if (config.dependencies) {
        const depIndex = config.dependencies.findIndex((dep: any) =>
