...
export async function executeOnline(
  ...
  try {
    ...
    if (!resourceId) {
      ...
      if (configFiles.hasResourceConfig || configFiles.hasCollectionConfig) {
              ...
              const { confirmUseConfig } = await inquirer.prompt([
                {
                  ...
                  message: `是否上架配置文件中的资源: ${chalk.cyan(resourceConfig.resourceName || resourceConfig.resourceId)} (ID: ${resourceConfig.resourceId})?`,
                  default: true,
                },
              ]);
              ...
              const { confirmUseConfig } = await inquirer.prompt([
                {
                  type: 'confirm',
                  name: 'confirmUseConfig',
                  message: `是否上架配置文件中的合集资源: ${chalk.cyan(collectionConfig.resourceName || collectionConfig.resourceId)} (ID: ${collectionConfig.resourceId})?`,
                  default: true,
                },
              ]);
        ...
        if (!resourceId) {
          ...
          console.log(chalk.cyan('  2. 使用命令参数: freelog-cli2 online <resourceId>'));
    ...
    handleErrorAndExit(err, '上架资源失败', options.debug);
  }
}
