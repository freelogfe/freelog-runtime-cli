...
| Step4 上架 | `step4Effects.ts` | `update`(tags, coverImages, intro, **status:1**) |
...
| Step4 | `step4Effects.ts` | `update` → **`setCollectRules`** → `update`(**status:1**) |
...
| 上下架 | `sidebar/Sider/index.tsx` | `info` | `update`(status:1 或 4) |
...
| 上架 | `resourceId`, `status: 1` | `step4Effects.ts`, `Sider/index.tsx` |
...
### Step 1：从 tools-lib 复制接口定义（本文档 §4 为参数依据）

按 `tools-lib/src/service-API/resources.ts` 逐函数移植，**以 Console 传参为验收标准**。

### Step 2：修复 8 个错误接口

优先修 `createBatch`（阻塞 batch 流程）。

### Step 3：补 P0 缺失（5 个）

### Step 4：更新 CLI commands 调用方

| CLI 命令 | 应对齐的 Console API |
|---------|---------------------|
| `create` | `create` + `getResourceTypeInfoByCode` |
| `publish` | `createVersion`（参数见 §4.1） |
| `syncr` / `pull` | `info`（§4.1 参数组合） |
| `syncv` / `pull` | `resourceVersionInfo1` |
| `update` | `update`（补 `resourceTitle`） |
| `batch create` | `createBatch`（非 batchCreateResources） |
| `collection publish` | `updateCollection`（含 isMergeCatalogueDraft） |
| `collection item add` | `addResourceItems_Draft` |
| `pull --collection` | `getCollectionItems_Draft` + `getCollectionCollectRules` |

### Step 5：对照测试

在测试环境用与 Console 相同的参数调用，比对响应结构。
