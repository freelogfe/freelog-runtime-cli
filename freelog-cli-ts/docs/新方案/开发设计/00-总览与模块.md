...
> 实现入口：[02-命令规格](./02-命令规格.md) · CLI 横切：[08](./08-CLI工程约定.md) · 业务×CLI：[09](./09-业务与CLI技术结合.md)
...
flowchart TD
  ...
  Val --> API[api]
  Adapters --> API
  API --> Write[原子写回config]
```

分层禁令与 CliError 见 [08-CLI工程约定](./08-CLI工程约定.md)。
...
| 版本 API | `src/api/version.ts` | POST/GET/DELETE drafts |
