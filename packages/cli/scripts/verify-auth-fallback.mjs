 ...
 *
 * 用法：pnpm verify:auth-fallback [--env dev]
 */
...
console.log(`\n=== authExcluded import-dir fallback (env=${env}) ===\n`);
...
try {
  ...
  workBase = fs.mkdtempSync(path.join(os.tmpdir(), 'freelog-auth-fallback-'));
  ...
  const detail =
    ...
    error.message;
  console.error('✘ auth fallback', detail);
} finally {
