...
import { TEMPLATE_DISPLAY_NAMES } from './initCatalog.js';
...
export async function pickInitTemplate(
  ...
  if (!rows.length) {
    throw new CliError('没有可用模板', { code: 4, hint: 'freelog-cli template list' });
  }
  ...
  if (p.isCancel(choice)) {
    throw new CliError('已取消模板选择', { code: 4 });
  }
...
export async function pickInitNamespace(): Promise<string> {
  ...
  if (p.isCancel(ns)) {
    throw new CliError('已取消 namespace 输入', { code: 4 });
  }
...
export async function pickInitResourceIdentity(
  ...
  if (p.isCancel(answers)) {
    throw new CliError('已取消资源信息输入', { code: 4 });
  }
  return {
