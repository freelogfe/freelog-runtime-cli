...
type MetaInfoArray = MetaInfoItem[];
...
function assertApiOk<T>(envelope: ApiEnvelope<T>, label: string): T {
  ...
  if (ret !== 0 || errCode !== 0) {
    throw new CliError(`${label} 失败: ${envelope.msg || '未知错误'}`, {
      code: 4,
      details: envelope,
    });
  }
  if (envelope.data === undefined) {
    throw new CliError(`${label} 返回空 data`, { code: 4, details: envelope });
  }
...
export function createVersionPropertiesFromHandleData(result: HandleFilePropertiesResult): {
...
async function loadMetaInfoArray(opts: {
  ...
  if (opts.sha1) {
    ...
    if (error) {
      throw new CliError(`文件属性解析失败: ${error}`, { code: 4, details: { sha1: opts.sha1 } });
    }
...
export async function resolveCreateVersionPropertiesFromFile(opts: {
  ...
  if (handleResult.state !== 'success') {
    throw new CliError(handleResult.failedMsg || '文件属性解析失败', {
      code: 4,
...
export async function resolveCollectionPropertiesFromType(opts: {
  ...
  if (handleResult.state !== 'success') {
    throw new CliError(handleResult.failedMsg || '合集类型属性模板加载失败', {
      code: 4,
