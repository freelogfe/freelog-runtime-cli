...
const ADS_LIST_PATH = '/v2/activities/ads/list4Client';
...
interface LotteryDrawParamsType {
  capsuleType: 1 | 2 | 3 ; // 第一阶段，第二阶段，第三阶段
  num: 3; // 抽取个数（填3）
}

export function lotteryDraw(params: LotteryDrawParamsType) {
  return FUtil.Request({
    method: 'POST',
...
export function lotteryDrawResult(params: LotteryDrawResultParamsType) {
  return FUtil.Request({
    method: 'GET',
