...
- 已审阅证据：`docs/一期/`、`packages/cli/src/`、Console 资源页、CLI 单元与场景验证脚本。
...
2. `docs/一期/产品方案/README.md`定义一期资源发行域的产品方案入口。
3. `docs/一期/产品方案/00-方案总览与验收标准.md`到`06-实现解决方案.md`共同定义需求分析、Console 业务流程、CLI 环境差异、产品原则、CLI 流程、字段约束、接口、异常恢复、实现分层和验收口径。
4. `docs/一期/产品方案/07-项目上下文与接续记录.md`只记录新会话接续上下文、证据入口、敏感信息边界和当前进展；不替代产品裁决。
5. `docs/一期/archive/`只保存旧拆分材料、旧验证报告和历史入口页；不作为当前产品或实现真源。

若上述细化文档与本文冲突，先修正文档，不得用“代码已经如此”替代产品决策。

### 官方使用文档交付契约

如未来重新拆分可交付给 Freelog 官方文档站的最终用户文档集合，必须满足：
...
同一套 Freelog 业务规则可通过两种 **本地 Store** 暴露（详见 [CLI 环境差异与产品原则](docs/一期/产品方案/03-CLI环境差异与产品原则.md)）：
...
3. 命令面与工程模式 **同名**，由 `--session` + `--resource-id` 激活；细节见 [CLI 流程与命令设计](docs/一期/产品方案/04-CLI流程与命令设计.md)。
...
**session 一词仅指 S=1**（不写 manifest/state），不表示凭据是否落盘。完整流程、userId 规则、studio 场景和验收边界见 [CLI 流程与命令设计](docs/一期/产品方案/04-CLI流程与命令设计.md) 与 [场景、异常与验收方案](docs/一期/产品方案/05-场景异常与验收方案.md)。
...
- 没有本地显式值和远端草稿时，新版本可以继承 latestVersion 的文件、描述、属性、直接依赖、基础上抛资源和授权排除项；继承 attrs 须按平台 descriptor 过滤（`insertMode` / `supportOptionalConfig`，见 [Console 业务流程、字段与接口](docs/一期/产品方案/02-Console业务流程字段接口.md)）；继承结果必须写入本地意图或在执行计划中完整展示。
...
该表与 [Console 业务流程、字段与接口](docs/一期/产品方案/02-Console业务流程字段接口.md) 和 [场景、异常与验收方案](docs/一期/产品方案/05-场景异常与验收方案.md) 必须同步。`CONTRACT` 只表示业务事实已核验，`ENV` 未完成时不得写“完整对齐”。
...
- **Prompting（TTY）：** 每个交互输入步骤须在用户键入 **之前** 展示该字段的 HARD 约束摘要；键入时使用与写平台前相同的校验器即时反馈。规格和 Console 字段事实统一见 [Console 业务流程、字段与接口](docs/一期/产品方案/02-Console业务流程字段接口.md) 和 [CLI 流程与命令设计](docs/一期/产品方案/04-CLI流程与命令设计.md)。
