...
adapters/*              纯形状转换（无 IO）
platform/service-api/*  ≅ FServiceAPI（tools-lib 同签名）
platform/request.ts     ≅ FUtil.Request（Node Bearer）
platform/tool/*         ≅ FUtil.Tool（含 getSHA1Hash）
...
flowchart TD
  ...
  Val --> SAPI[ServiceAPI]
  Adapters --> SAPI
  SAPI --> NReq[NodeRequest]
  Svc --> Tool[PlatformTool_getSHA1Hash]
...
| 平台 API | `src/platform/service-api/*` | ≅ FServiceAPI；含 drafts POST/GET/DELETE |
| SHA1 等 Tool | `src/platform/tool/getSHA1Hash.ts` | ≅ FUtil.Tool.getSHA1Hash |
