...
> **立场**：以 2026 Node CLI 主流实践 + **与 Console 同源契约** 定稿；**不以**当前 `freelog-cli-ts` 依赖/目录为约束。  
> 落地方案若与现状分叉，实现期按本文件迁移，而不是反过来改口径。  
> 产品对照 → [05-Console与CLI对照](../产品设计/05-Console与CLI对照.md) · API 清单 → [API/API迁移清单](./API/API迁移清单.md)
...
| 模块 | **ESM 优先**（`"type": "module"`） | 2026 CLI 默认；CJS 仅作过渡出口若需要 |
...
| 「旧 api 打 `@deprecated` 长期留着」 | 里程碑内删掉 |
